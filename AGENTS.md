...
- `src/cache.rs` / `src/fetch.rs`: refresh `data/dictionary.json` from `/v2/items` when missing, unreadable, malformed, or at least 72 hours old. Store slugs, English names (fallback: slug), tags, and optional vaulted/ducat metadata.
...
- Keep only rows with absent or numeric-zero `rank`; strip `id`, `datetime`, and `order_type` from output rows. Liquidity sums today's and yesterday's volumes.
